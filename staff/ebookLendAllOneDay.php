<?php
    ...
    $sql = "SELECT DISTINCT(lend.isbn),COUNT(lend.isbn),book.name,book.price,book.thumbnail,invoice.date_pay
            FROM lend,invoice,book
            WHERE invoice.date_pay='2021-12-01' AND lend.invoice_id=invoice.id AND book.isbn=lend.isbn
            GROUP BY lend.isbn";
    $data = executeResult($sql);
?>
<h3 style="margin:70px 350px 0">Xem tổng số sách điện tử được thuê trong một ngày 2021-12-01</h3>
<table class="table">
  <thead>
    <tr>
      <th scope="col">STT</th>
  ...
  <tbody>
    <?php
        for($i=0;$i<count($data);$i++){
            ...
            echo '<td>'.$data[$i]["COUNT(lend.isbn)"].'</td>';
